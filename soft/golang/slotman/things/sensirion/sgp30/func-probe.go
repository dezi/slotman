...
func ProbeThings(busyDevicePaths []string) (things []*SGP30, err error) {
	...
	for _, devicePath := range devicePaths {
		...
		xfer, tryErr := i2cDev.WriteBytes([]byte{0x20, 0x32})
		log.Cerror(tryErr)
		if tryErr == nil && xfer == 2 {
			...
			if tryErr == nil && xfer == 3 {
				if result[2] == calculateCrc(result[0:2]) && result[0] == 0xd4 {
					log.Printf("Identified SGP30 devicePath=%s result=[ %02x ]", devicePath, result)
					isValid = true
				}
			}
		}
		...
		if isValid {
			things = append(things, NewSG30(deviceAddrPath))
		}
	}
